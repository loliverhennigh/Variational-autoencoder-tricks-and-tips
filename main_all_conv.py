...
def train():
  ...
  with tf.Graph().as_default():
    ...
    # conv4
    conv4 = ld.conv_layer(conv3, 3, 2, 2, "encode_5", True)
    ...
    for step in xrange(FLAGS.max_step):
      ...
      if step%500 == 0:
        ...
        kl_loss_dis = np.sort(np.sum(kl_loss_dis.reshape(FLAGS.batch_size, 16), axis=0))
        stddev_r = np.sort(np.sum(stddev_r.reshape(FLAGS.batch_size, 16), axis=0))
